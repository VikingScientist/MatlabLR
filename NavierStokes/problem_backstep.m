...
'Save_Results'      ,  false,       ...
'Time_Step'         ,  .005,        ...
'Boundary_Startup'  ,  [0,.02],        ...
'Time_Startup_Steps',  111,        ...
'Time_Integrator'   ,  'CN',        ...
'Time_Range'        ,  [0,3]);

BC     = cell(0);
%%% Normal velocity boundary conditions (strongly enforced)
...
%%% Tangengital velocity boundary conditions (strong)
BC = [BC, struct('start', [-4,2], 'stop', [10,2], 'comp', 1, 'value', 0)]; % horizontal top edge
BC = [BC, struct('start', [0,-1], 'stop', [10,-1],'comp', 1, 'value', 0)]; % horizontal bottom edge
BC = [BC, struct('start', [-4,0], 'stop', [0,0],  'comp', 1, 'value', 0)]; % horizontal corner edge
% BC = [BC, struct('start', [-4,0], 'stop', [-4,2], 'comp', 2, 'value', 0)]; % vertical left edge
BC = [BC, struct('start', [0,-1], 'stop', [0,0],  'comp', 2, 'value', 0)]; % vertical corner edge
% BC = [BC, struct('start', [10,-1],'stop', [10,2], 'comp', 2, 'value', 0)]; % vertical right edge

%%% Tangengital velocity boundary conditions (weak)
...
%%% Neumann boundary conditions (specify the force, i.e. stress tensor at the boundar)
%%%   sigma = (2*mu*nabla^S(u) -pI)*n \in R^{2}
BC = [BC, struct('start', [-4,0], 'stop', [-4,2], 'neumann', true, 'value', [1;0])]; % vertical left edge

%%% Pressure edges
% BC = [BC, struct('start',  [-4,0], 'stop', [-4,2], 'comp', 3, 'value', 10)]; % vertical left edge (inflow)
% BC = [BC, struct('start', [10,-1], 'stop', [10,2], 'comp', 3, 'value', 0)]; % vertical right edge (outflow)

% collocation corner points
e = 1e-10;
BC = [BC, struct('collocation', true, 'u', -4+e, 'v', e)];
BC = [BC, struct('collocation', true, 'u', -4,   'v', 2)];
BC = [BC, struct('collocation', true, 'u',  e,   'v', e)];
BC = [BC, struct('collocation', true, 'u',  e,   'v',-1+e)];
BC = [BC, struct('collocation', true, 'u',10-e,  'v',-1+e)];
BC = [BC, struct('collocation', true, 'u',10-e,  'v', 2)];
