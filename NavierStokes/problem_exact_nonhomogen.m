...
Re = 13; % Reynolds number.   May be needed in Problem.Time_Step
...
'Newton_TOL'        ,  1e-10,      ...
'Newton_Max_It'     ,  12,         ...
'Force'             ,  @(x,y) ... %[(x^2 + y^2)/(x + y + 1) + 2*x*log(x + y + 1); (x^2 + y^2)/(x + y + 1) + 2*y*log(x + y + 1)], ... % linear stokes
[(x^2 + y^2)/(x + y + 1) + 2*x*log(x + y + 1) + exp(2*x)*cos(y)^2 + exp(2*x)*sin(y)^2; (x^2 + y^2)/(x + y + 1) + 2*y*log(x + y + 1)],... % non-linear
'Static'            ,  true,       ...
'Linear'            ,  false,       ...
'Paraview'          ,  false,      ...
'MatlabPlot'        ,  false,       ...
...
'grad_u',@(x,y) [ exp(x).*cos(y); ...
                 -exp(x).*sin(y)], ...
'grad_v',@(x,y) [-exp(x).*sin(y); ...
                 -exp(x).*cos(y)]);

BC     = cell(0);
BC = [BC, struct('pressure_integral', true, 'value', log((81*3^(1/2))/16) - 59/36)];
