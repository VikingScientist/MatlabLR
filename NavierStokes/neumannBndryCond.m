...
for edge=1:numel(BC)
  bc = BC{edge};
  if ~isfield(bc, 'neumann') || bc.neumann==false % only parse neumann boundaries
    continue;
  end
  if bc.start(1) == bc.stop(1)     % vertical line   (const. xi)
    ...
    elseif numel(elmEnd) > 0
      ...
      disp 'Error: neumannEnforceBC, edge not found with any corresponding elements'
      bc
      break;
    end
  ...
  elseif bc.start(2) == bc.stop(2) % horizontal line (const. eta)
    ...
    elseif numel(elmEnd) > 0
	  ...
	  disp 'Error: neumannEnforceBndryCond requests only horizontal or vertical input lines';
    break;
  end
    ...
    if running_param == 'v'
      for i=1:nGauss,
        ...
        if isa(bc.value, 'function_handle')
          traction_bc = bc.value(map.x(1), map.x(2));
        else
          traction_bc = bc.value;
        end
        
        b(globIvel) = b(globIvel) + testVel'*traction_bc *detJw;
